...
function initialize() {
  ...
  const dateEnd = new Date(data.lastupdated[0]).toLocaleString("sv-SE", {"timeStyle": "short"});
...
function renderMaps(maps) {
  ...
  for (let i = 0; i < maps.length; i++) {
    imgblock += `<img src="https://mapartwall.rebane2001.com/mapimg/map_${maps[i]}.png?v=${data.lastupdated[0]}" loading="lazy" class="mapartimg" width="128" height="128" title="ID: ${maps[i]}" alt="ID: ${maps[i]}" onclick="mapClick(${maps[i]})" onerror="reloadImage(this)" />`;
  }
  document.getElementById('maparts').innerHTML = imgblock;
}

function mapClick(id) {
  modalimg.src = "https://mapartwall.rebane2001.com/mapimg/map_" + id + ".png?v=" + data.lastupdated[0];
